...
with open('E:finding_mutations/dataset_26255_10.txt', 'r') as f:
    ...
    for i in range(len(states_test)):
        for j in range(len(emissions_test)):
            emission_matrix_test[states_test[i]+emissions_test[j]
                                 ] = float(temp[i+1].split('\t')[j+1])
print(outcome_probability(emission_outcome_test,
      hidden_path_test, emission_matrix_test))
